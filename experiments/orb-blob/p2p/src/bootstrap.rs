...
impl Bootstrapper {
    pub async fn find_bootstrap_peers(
        ...
        let mut bootstrap_nodes: BTreeSet<EndpointAddr> =
...
mod test {
    ...
    async fn test_bootstrap_http() {
        let mut example_addrs: Vec<_> = example_keys(8)
            .iter()
            .map(SecretKey::public)
            .map(EndpointAddr::from)
            .collect();
        let well_known_nodes = example_addrs.split_off(4);
        let http_nodes = example_addrs;
        let all_expected_nodes: BTreeSet<EndpointAddr> = http_nodes
            .clone()
            .into_iter()
