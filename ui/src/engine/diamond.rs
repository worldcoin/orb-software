...
use crate::engine::{animations, operator, Animation, AnimationsStack, CenterFrame, ConeFrame, Event, EventHandler, OperatingMode, OperatorFrame, OrbType, QrScanSchema, QrScanUnexpectedReason, RingFrame, Runner, RunningAnimation, SignupFailReason, Transition, UiMode, UiState, DIAMOND_CENTER_LED_COUNT, DIAMOND_CONE_LED_COUNT, DIAMOND_RING_LED_COUNT, LED_ENGINE_FPS, LEVEL_BACKGROUND, LEVEL_FOREGROUND, LEVEL_NOTICE};
use crate::sound;
...
impl EventHandler for Runner<DIAMOND_RING_LED_COUNT, DIAMOND_CENTER_LED_COUNT> {
    ...
    fn event(&mut self, event: &Event) -> Result<()> {
        match event {
            ...
            Event::BootComplete { api_mode } => {
                ...
                self.state = UiState::Booted(if *api_mode { UiMode::Api } else {UiMode::Core});
    ...
    async fn run(&mut self, interface_tx: &mut Sender<Message>) -> Result<()> {
        ...
        let paused = if let UiState::Paused(_) = self.state {
            true
        } else {
            false
        };
