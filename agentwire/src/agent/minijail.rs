...
pub enum DefaultAction {
    /// Allow blocked syscalls but log them (development/debugging only).
    /// Uses `SECCOMP_RET_LOG` - the syscall proceeds but is logged to the kernel
    /// audit log. Useful for discovering which syscalls an agent actually needs.
    /// **WARNING**: Not secure for production - blocked syscalls still execute!
    Log,
    /// Immediately terminate the process (default) - maximum security.
    /// Uses `SECCOMP_RET_KILL` - the process dies immediately on any unlisted syscall.
    #[default]
    Kill,
}
...
fn generate_seccomp_policy(policy: &SeccompPolicy) -> String {
    ...
    match policy.default_action {
        DefaultAction::Log => output.push_str("log (allow but audit)\n"),
...
pub fn apply_minijail(
    ...
    // 4. Configure default action for unlisted syscalls
    // NOTE: log_seccomp_filter_failures() MUST be called before parse_seccomp_filters()
    if matches!(seccomp_policy.default_action, DefaultAction::Log) {
        // Log mode: blocked syscalls are allowed but logged to kernel audit log.
        // Useful for development to discover which syscalls an agent needs.
        // WARNING: Not secure - syscalls still execute!
        jail.log_seccomp_filter_failures();
    }
    // Kill mode (default): blocked syscalls terminate the process immediately.

    // 5. Parse and apply seccomp filter
...
mod tests {
    ...
    fn test_default_action_log() {
        ...
        let output = generate_seccomp_policy(&policy);
        assert!(output.contains("Default action: log"));
    }
    ...
    fn test_seccomp_policy_default() {
        ...
        assert!(matches!(policy.default_action, DefaultAction::Kill));
    }
}
