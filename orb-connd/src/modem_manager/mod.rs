...
impl ModemManager {
    pub async fn list_modems(&self) -> color_eyre::eyre::Result<Vec<Modem>> {
        let output = run_cmd("mmcli", &["-L"]).await?;
        Ok(parse_mmcli_modem_list(&output))
    }

    pub async fn modem_info(
        &self,
        modem_id: &ModemId,
    ) -> color_eyre::eyre::Result<ModemInfo> {
        let output = run_cmd("mmcli", &["-m", modem_id.as_str(), "-J"]).await?;
        parse_modem_info(&output)
    }

    pub async fn signal_setup(
        &self,
        modem_id: &ModemId,
    ...
    pub async fn sim_info(&self, sim_id: &SimId) -> color_eyre::eyre::Result<SimInfo> {
    ...
    pub async fn set_allowed_and_preferred_modes<'a>(
        &self,
        modem_id: &ModemId,
        allowed: &[&'a str],
        preferred: &'a str,
...
mod tests {
    ...
    use crate::modem_manager::{
        connection_state::ConnectionState, parse_location, parse_mmcli_modem_list,
        parse_signal, Location, Modem, ModemId, ModemInfo, Signal, SimId, SimInfo,
    };
