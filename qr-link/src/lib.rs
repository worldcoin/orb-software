...
//! # Usage
//!
//! # Old Flow (with session creation using signup-service)
//! ## Encode (App)
//!
...
//!
//! # New flow
//! * Once the new flow is rolled out - we can remove the optional parameters
//! ## Encode (App)
//!
//! Worldcoin App generates a QR-code using the orb relay ID and includes a hash of the user data.
//!
//! ```rust
//! use orb_qr_link::{encode_qr, DataPolicy, UserData};
//! use uuid::Uuid;
...
//! let user_data_hash = user_data.hash(16);
//! // Encode a new QR-code.
//! let qr = encode_qr(&orb_relay_id, user_data_hash);
//!
//! // Allow the Orb to scan the generated QR-code.
//! // ...
//! ```
...
//!
//! // Decode the QR-code string.
//! let (orb_relay_id, user_data_hash) = decode_qr(qr).unwrap();
//!
//! // connects to the app via orb relay using the orb_relay_id
//!
//! // Retrieves the user data from the AnnounceAppId Orb Relay Message
//! let user_data = UserData {
//!     identity_commitment: String::new(),
//!     self_custody_public_key: String::new(),
//! };
