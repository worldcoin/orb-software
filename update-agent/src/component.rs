    ...
    fn test_diffing_self_then_patching_produces_same_output() -> Result<()> {
        ...
        let examples: Result<Vec<_>> = examples
            ...
            .map(|(file_name, contents)| {
                let sqfs_path = tmp_dir.path().join(file_name).with_extension("sqfs");
        ...
        for (sqfs_path, _sqfs_contents) in examples {
            let sqfs_contents = std::fs::read(&sqfs_path)
                .wrap_err_with(|| format!("failed to read {}", sqfs_path.display()))?;
            let patch_path = sqfs_path.with_extension("patch");
            let after_patch_path = sqfs_path.with_extension("after_patch.sqfs");

            let mut patch_file = std::fs::File::create_new(&patch_path)
                .wrap_err("failed to create empty patch file")?;

            println!("before diff: {sqfs_path:?}");
            let foo = wip_bidiff::diff_squashfs(
                &sqfs_path,
                &sqfs_contents,
                &sqfs_path,
                &sqfs_contents,
                &mut patch_file,
                &wip_bidiff::DiffParams::default(),
            )
            .wrap_err("failed to perform diff");
            println!("after diff: {foo:?}");
            println!();
            println!();
            println!();
            foo?;
