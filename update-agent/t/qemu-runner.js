...
async function copyOvmfFiles(dir) {
  ...
  const isUbuntu =
    ovmfPaths.codePath.includes("OVMF") && ovmfPaths.codePath.endsWith(".fd");
  ...
  Logger.info(
    `Copying OVMF code from ${ovmfPaths.codePath} to mock directory...`,
  );
  await fs.copyFile(ovmfPaths.codePath, ovmfCodeDestPath);

  Logger.info(
    `Copying OVMF vars from ${ovmfPaths.varsPath} to mock directory...`,
  );
...
async function populateMockMnt(dir) {
  ...
  const bootImg = join(mntDir, "boot.img");
  const systemImg = join(mntDir, "system.img");
  const fedoraCloudImage = join(dir, "fedora-cloud.qcow2");

  await fs.writeFile(bootImg, "boot");

  Logger.info("Converting Fedora qcow2 image to raw format for system.img...");
  const qemuImgResult = Bun.spawnSync([
    "qemu-img",
    "convert",
    "-f",
    "qcow2",
  ...
  if (!qemuImgResult.success) {
    throw new Error(
      `Failed to convert qcow2 to raw: ${qemuImgResult.stderr?.toString()}`,
    );
  }

  Logger.info("Calculating hash of system.img...");
  // Calculate hash and size using chunked reads
  const systemImgHandle = await fs.open(systemImg, "r");
  const systemImgStats = await systemImgHandle.stat();
  const systemSize = systemImgStats.size;

  const hasher = new Bun.CryptoHasher("sha256");
  const chunkSize = 64 * 1024 * 1024; // 64MB chunks
  let bytesRemaining = systemSize;
  let currentOffset = 0;
  ...
  while (bytesRemaining > 0) {
    ...
    await systemImgHandle.read(buffer, 0, currentChunkSize, currentOffset);
  ...
  await systemImgHandle.close();
  const systemHash = hasher.digest("hex");
  const bootHash = createHash("sha256").update("boot").digest("hex");

  // Deleted the MCUs since their consumption is out of scope for this
  // E2E test.
  const claimData = {
    status: "update_available",
    version: "0.0.0-qemu-diamond-dev",
    manifest: {
      magic: "take this magic",
      type: "normal",
      components: [
        {
          "version-assert": "idk2",
...
async function createMockDisk(dir) {
  ...
  const partedCommands = [
    ["mklabel", "gpt"],
    ["mkpart", "A_BOOT", "1M", "17M"],
    ["mkpart", "B_BOOT", "17M", "33M"],
    ["mkpart", "esp", "33M", "97M"],
    ["set", "3", "esp", "on"],
    ["mkpart", "A_SYSTEM", "97M", "8289M"],
    ["mkpart", "B_SYSTEM", "8289M", "16481M"],
  ...
  for (const cmd of partedCommands) {
    ...
    if (!result.success) {
      ...
      throw new Error(
        `parted command failed: ${cmd.join(" ")}\nstdout: ${stdout}\nstderr: ${stderr}`,
      );
    }
  }
}
...
async function createImageFromDirectory(sourceDir, imagePath, sizeInMB) {
  ...
  if (!result.success) {
    throw new Error(
      `mkfs.ext4 failed for ${imagePath}: ${result.stderr?.toString()}`,
    );
  }
}
  ...
  try {
    ...
    for await (const chunk of response.body) {
      ...
      if (contentLength > 0 && process.stdout.isTTY) {
        ...
        Logger.info(
          `Download progress: ${progress}% (${downloadedBytes}/${contentLength} bytes)`,
        );
      }
    }
...
async function createCloudInit(dir) {
...
write_files:
  - path: /etc/systemd/system/worldcoin-update-agent.service
    content: |
      ...
      [Service]
      Type=simple
      ExecStart=/mnt/program/update-agent --id qemu-mock
      RemainAfterExit=no
...
`;
  ...
  const genisoimageProcess = Bun.spawnSync(
    [
      "genisoimage",
      "-output",
      cloudInitIso,
  ...
  if (!genisoimageProcess.success) {
    throw new Error(
      `genisoimage failed with code ${genisoimageProcess.status}`,
    );
  }
...
async function runQemu(programPath, mockPath) {
  ...
  // Create the cloud-init ISO for this run.
  const cloudInitIso = await createCloudInit(absoluteMockPath);
  ...
  const qemuArgs = [
    ...
    "-nographic",
    "-drive",
    `if=pflash,format=${ovmfFormat},file=${actualCodePath},readonly=on`,
    "-drive",
    `if=pflash,format=${ovmfFormat},file=${actualVarsPath}`,
    "-drive",
    `file=${cloudImagePath},format=qcow2,if=virtio,snapshot=on`,
    "-drive",
    `file=${join(absoluteMockPath, "disk.img")},format=raw,if=none,id=nvme-img`,
    "-device",
    "nvme,drive=nvme-img,serial=mock-nvme",
    "-drive",
    `file=${cloudInitIso},format=raw,if=virtio,readonly=on`,
    "-drive",
    `file=${mntImg},format=raw,if=virtio,snapshot=on`,
    "-netdev",
    "user,id=net0",
    "-device",
    "virtio-net-pci,netdev=net0",
    "-virtfs",
    `local,path=${programDir},mount_tag=program,security_model=passthrough,id=program`,
    "-serial",
    "mon:stdio",
  ];
  ...
  try {
    ...
    await qemuProcess.exited;
  }
}

async function compareComponent(mockPath, name, partlabel) {
  const diskPath = join(mockPath, "disk.img");
  const sourcePath = join(mockPath, "mnt", `${name}.img`);

  const diskInfoResult = Bun.spawnSync([
    "parted",
    "--json",
    "--script",
    diskPath,
  ...
  if (!diskInfoResult.success) {
    throw new Error(
      `Failed to get partition info: ${diskInfoResult.stderr?.toString()}`,
    );
  }

  const diskInfo = JSON.parse(diskInfoResult.stdout.toString());
  let partitionStart = null;
  let partitionSize = null;

  for (const partition of diskInfo.disk.partitions) {
    if (partition.name === `B_${partlabel}`) {
      partitionStart = parseInt(partition.start.replace("B", ""));
      partitionSize = parseInt(partition.size.replace("B", ""));
      break;
    }
  }

  if (partitionStart === null) {
    throw new Error(`Could not find B_${partlabel} partition`);
  }

  Logger.info(
    `Found B_${partlabel} partition at offset ${partitionStart}, size ${partitionSize} bytes`,
  );

  const sourceStats = await fs.stat(sourcePath);
  const sourceSize = sourceStats.size;

  Logger.info(`${name}.img size: ${sourceSize} bytes`);

  const diskHandle = await fs.open(diskPath, "r");
  const sourceHandle = await fs.open(sourcePath, "r");

  const chunkSize = 64 * 1024 * 1024; // 64MB chunks
  let bytesRemaining = sourceSize;
  let currentDiskOffset = partitionStart;
  ...
  try {
    while (bytesRemaining > 0) {
      ...
      const sourceBuffer = Buffer.alloc(currentChunkSize);
      ...
      if (!diskBuffer.equals(sourceBuffer)) {
        throw new Error(
          `B_${partlabel} partition does NOT match ${name}.img at offset ${currentDiskOffset - partitionStart}`,
        );
      }

      bytesRemaining -= currentChunkSize;
      currentDiskOffset += currentChunkSize;
      currentSourceOffset += currentChunkSize;
    }

    Logger.info(`✓ B_${partlabel} partition matches ${name}.img`);
    return true;
  } finally {
...
async function compareResults(mockPath) {
  ...
  return (
    (await compareComponent(mockPath, "boot", "BOOT")) &&
    (await compareComponent(mockPath, "system", "SYSTEM"))
  );
}
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log("Usage:");
    console.log(
      "  ./qemu-runner.js mock <dir>        - Create mockup directory",
    );
    console.log(
      "  ./qemu-runner.js run <prog> <dir>  - Run update-agent in QEMU",
    );
