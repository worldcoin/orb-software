...
fn new_fsc(
    ...
    mngr.camera_mut(cam_h, |cam| {
        let cam = cam.unwrap();
        if !cam.is_paired() {
            bail!("Camera should be paired first, before performing a flat scene calibration");
        }
        ...
        cam.store_flat_scene_correction(FlatSceneCorrectionId::_0, Some(|pct| BAR.get_or_init(|| ProgressBar::new(100)).set_position(pct as u64)))?;
