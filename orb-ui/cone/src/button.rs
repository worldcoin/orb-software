...
use color_eyre::eyre;
use ftdi_embedded_hal::libftd2xx::{BitMode, Ft4232h, FtdiCommon};
use std::sync::mpsc;
use tracing::debug;
...
impl Button {
    pub(crate) fn new(event_queue: mpsc::Sender<ConeEvents>) -> eyre::Result<Self> {
        let mut gpio = Ft4232h::with_serial_number("FT80R36LD")?;
        let mask: u8 = !BUTTON_GPIO_MASK; // button pin as input, all others as output
        gpio.set_bit_mode(mask, BitMode::AsyncBitbang)?;
