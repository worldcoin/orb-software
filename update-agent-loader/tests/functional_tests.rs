...
fn test_cli_args_parsing() {
    ...
    let result = cmd
        .args(["--url", "https://example.com/binary", "--args", "arg1", "arg2", "--help"])
        .assert();
    
    // The help output should contain our argument descriptions
    result
        .success()
        .stdout(predicate::str::contains("URL to download the executable from"))
        .stdout(predicate::str::contains("Arguments to pass to the executable"));
}
